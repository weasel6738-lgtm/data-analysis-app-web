...
3. 확인 체크박스를 선택하고 `정리 ZIP 생성`을 누릅니다.
4. ZIP을 내려받아 결과와 `manifest.csv`를 확인합니다.

지원 형식은 TXT, MD, CSV, XLSX, DOCX, PDF 및 일반 이미지입니다. 이미지는
OCR 없이 확장자로 분류합니다. 분류는 계약서, 영수증·세금계산서, 보고서,
회의록, 데이터, 이미지, 기타/분류불가 규칙을 사용하며 화면에서 키워드를
수정할 수 있습니다. 외부 AI나 네트워크로 문서 내용을 보내지 않습니다.
